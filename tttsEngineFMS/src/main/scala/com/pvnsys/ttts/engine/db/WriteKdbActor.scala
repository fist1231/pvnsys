...
object WriteKdbActor extends LazyLogging {
  ...
  def resetEngineData(tableId: String) = {
      ...
      val createEngineStr = s"engine$tableId" + """:([]funds:`float$();balance:`float$();transnum:`long$();intrade:`boolean$();possize:`long$();price:`float$();islong:`boolean$())"""
      val createTradeStr = s"trade$tableId" + """:([]dts:`datetime$();sym:`symbol$();price:`float$();size:`long$();oper:`symbol$();cost:`float$();balance:`float$();pl:`float$();plac:`float$();transnum:`long$())"""

      conn.k(createEngineStr)
 	  logger.debug("WriteKdbActor created ENGINE table: {}", createEngineStr)
      
 	  val insertEngineDataStr = s"`engine$tableId insert($funds;$balance;$transnum;$intradeStr;$possize;$price;$islongStr)"
...
//      val res = conn.k(s"update engine set funds=${data._1}, balance=${data._2}, transnum=${data._3}, intrade=${data._4}, possize=${data._5}")
      ...
      val updateStr = s"engine$tableId:update funds:${data._1},balance:${data._2},transnum:${data._3},intrade:${intradeStr},possize:${data._5},price:${data._6},islong:${islongStr} from engine$tableId" 
      conn.k(updateStr)
  ...
  def setTransactionData(tableId: String, data: TransactionKdbType) = {
      val conn: c = new c(Configuration.kdbHost, Configuration.kdbPort.toInt)
      val updateStr = s"`trade$tableId insert(${data._1};`${data._2};${data._3};${data._4};`${data._5};${data._6};${data._7};${data._8};${data._9};${data._10})" 
...
class WriteKdbActor(tableId: String) extends Actor with ActorLogging {
  ...
  def resetEngineData() = {
      ...
      conn.k(createEngineStr)
 	  ...
 	  val insertEngineDataStr = s"`engine$tableId insert($funds;$balance;$transnum;$intradeStr;$possize;$islongStr)"
 	  conn.k(insertEngineDataStr)
