...
//									  engineExecActor ! x
//								  }
//								  case None => logger.debug("No such Engine to process. Key {}", x.asInstanceOf[ResponseStrategyFacadeTopicMessage].client)
//								}
...
//						        strategies.get(x.asInstanceOf[ResponseStrategyServicesTopicMessage].client) match {
//								  case Some(engineExecActor) => {
